...
* > ./test
*
* Warning: you may have a different version of opencv so might have to change 
* "opencv4" to something else
*
*
*/
#include<iostream>
...
****************************************************************************/
	...
	//bigC is made of c_1' and c_2' (' means transpose)
	//Do SVD or turn bigC into a homogenous matrix that is square and invertible,
	//Both are the same but using a homogenous matrix Ab makes it easier to take the inverse of Ab.

/*	//SVD method, uncomment to use
	float bigC[] = { c_1.at<float>(0),c_1.at<float>(1),c_1.at<float>(2),
				 c_2.at<float>(0),c_2.at<float>(1),c_2.at<float>(2)};
	Mat Abb(Size(2,3), CV_32FC1, bigC);

	//Calculate SVD of Abb to use later
	Mat sigma, you, veet;
	SVD::compute(Abb, sigma, you, veet);
*/

	float bigC_homogenous[] = { c_1.at<float>(0),c_1.at<float>(1),c_1.at<float>(2),
					 c_2.at<float>(0),c_2.at<float>(1),c_2.at<float>(2),
					 0,0, 1 };
	Mat Ab(Size(3,3), CV_32FC1, bigC_homogenous);
	Mat Abinv = Ab.inv();
...
/*
			//Using SVD works same as using inverse of homogenous system
			Mat coriginal;
			SVD::backSubst(sigma, you, veet, output, coriginal);
*/

			Mat coriginal = Abinv*output;//method using inverse of homogenous Ab
